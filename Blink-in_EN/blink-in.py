...
def update_info():
    '''update information for every 5 mins'''
    ...
    with open('./config/diary.txt', 'r')as diary:
        diary = diary.read().replace('\n', '<br>')
...
@app.before_request
def before_request():
    '''handle scripts/aggressive requests'''
    global timing, update_timer

    if time() - update_timer > 1*60:
        update_info()
        update_timer = time()
        timing.clear()
    ...
    if target != 'http://blink-in.duckdns.org/favicon.ico':
        ...
        if(ip in ip_log.keys() and ((request.cookies.get('blink-in') != ip_log[ip][0]) or (head != ip_log[ip][1]))):
            return 'an device is currently using this ip @%s' % ip_log[ip][1]
...
def server_update():
    ...
    if server_config_type == 'offline':
        server_config_value.save('./offline_apps/%s' %
                                 server_config_value.filename)
...
@app.route('/members_work', methods=['GET'])
def members_work():
    return render_template('image_folder_member.html')


@app.route('/online_tools', methods=['GET'])
def online_tools():
...
def user_comments():
    ...
    ip = request.environ.get('REMOTE_ADDR')
    login_cookie = request.cookies.get('admin')
    if request.cookies.get('blink-in') == ip_log[ip][0]:
        if (request.method == 'GET'):
            with open('./server_log/comments.txt', 'r')as comments_log:
                comments = comments_log.read().replace('\n', '<br>')
            return comments1+comments+comments2
        else:
            ...
            if user_comment != '' and len(user_comment) >= 2:
                if login_cookie in admin_cookie:
                    with open('./server_log/comments.txt', 'a')as comments_holder:
                        comments_holder.write('Admin:' + user_name +
                                              '\t@%s: ' % ctime(time())+'\n'+user_comment+'\n')
                else:
                    with open('./server_log/comments.txt', 'a')as comments_holder:
                        comments_holder.write('user:' + user_name +
                                              '\t'+'('+ip+')'+'\t@%s: ' % ctime(time())+'\n'+user_comment+'\n')

    return redirect('/user_comments')


@app.route('/study_notes', methods=['GET'])
def study_notes():
    return render_template('note.html')


@app.route('/future_plan', methods=['GET'])
def future_plan():
...
if __name__ == '__main__':
    try:
        app.run(host='0.0.0.0', port=443, ssl_context=(
            r'C:\Certbot\live\blink-in.tk\fullchain.pem', r'C:\Certbot\live\blink-in.tk\privkey.pem'))
    except:
